...
### 【推奨】Claude Code / Codex に任せる

環境差 (uv 未導入 / Ollama 未起動 / 権限が下りていない など) でつまずきやすいので、
**AI コーディングエージェントに丸投げするのが一番確実**です。作業用フォルダで
`claude` または `codex` を起動して、こう伝えてください:

```
https://github.com/tomoyaoshima/voiceinput をクローンして、
README を読んで macOS にセットアップしてください。

- 私はエンジニアではないので、専門用語は噛み砕いて説明してください
- 前提ツール(uv / Ollama)が未導入なら、それも入れてください
- 私がクリックする必要がある操作(macOS の権限許可など)は、
  どの画面で何をすればいいか具体的に指示してください
- 途中でエラーが出たら、原因を調べて直してから次に進んでください
- 最後に、実際に起動して音声入力が動くところまで確認したいです
```

このプロジェクト自体が Claude Code との対話だけで作られており、AI が読んで
理解しやすい構成・ドキュメントになっています。
...
| 整形モード切替 | LLM モデル切替 | STT モデル切替 |
|---|---|---|
| ![Format](docs/images/menu-format.jpg) | ![LLM Model](docs/images/menu-llm-model.jpg) | ![STT Model](docs/images/menu-stt-model.jpg) |
...
このリポジトリは「完成品」ではなく **「たたき台」** です。MIT ライセンスなので改造は自由。
そして改造も Claude Code / Codex に相談すれば実現できます。

```
このリポジトリを読んで、Slack に投稿するときだけ絵文字まじりの
カジュアルな文体に自動整形されるようにしてください。
```
```
議事録モードを追加したいです。喋った内容を箇条書きにして、
決定事項と TODO を分けて出力してほしいです。
```
```
ホットキーを右 Command から別のキーに変えたいです。
```

整形モードは `prompts/` に YAML を 1 つ足すだけで追加できます
(`name` / `system` / `user_template` の 3 つを書くだけ)。
`config.yaml` の `app_mode` を使えば「このアプリではこのモード」の自動切替もできます。

**「不満を言葉にする → AI が実装する」** を繰り返して、自分の仕事に最適化していってください。
良いものができたら Pull Request も歓迎です。
